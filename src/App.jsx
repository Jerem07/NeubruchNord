...
const supabase = createClient(
  "https://yocoqkcblrxwrqesqhng.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlvY29xa2NibHJ4d3JxZXNxaG5nIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODIzMTkyNzUsImV4cCI6MjA5Nzg5NTI3NX0.BTRPj_DDYO1wodhitTTtw8jkB16OHwo1-5A0foZeij8"
);
...
export default function App() {
  ...
  const pollSession = useCallback(async () => {
    ...
    try {
      const { data: s } = await supabase.from("sessions").select("*").eq("id", currentSession).eq("mode", appMode).single();
      const { data: m } = await supabase.from("members").select("*").eq("session_id", currentSession);
      const { data: i } = await supabase.from("items").select("*").eq("session_id", currentSession).order("logged_at", { ascending: true });
  ...
  useEffect(() => {
    if (appMode === "game") {
      supabase.from("rounds").select("*").order("position", { ascending: true }).then(({ data }) => { if (data) setRounds(data); });
    }
  }, [appMode]);
  ...
  async function createSession() {
    ...
    try {
      ...
      const { error: e1 } = await supabase.from("sessions").insert(session);
      if (e1) throw e1;
      const { error: e2 } = await supabase.from("members").insert({ session_id: code, name: nameInput.trim(), score: 0 });
      if (e2) throw e2;
      setSessionData(session);
      setMembersData([{ session_id: code, name: nameInput.trim(), score: 0 }]);
      setItemsData([]);
  ...
  async function joinSession() {
    ...
    setLoading(true);
    try {
      const { data: s, error: e1 } = await supabase.from("sessions").select("*").eq("id", code).eq("mode", appMode).single();
      if (e1 || !s) { setError("Session introuvable"); setLoading(false); return; }
      if (s.ended || Date.now() > s.ends_at) { setError("Session terminée"); setLoading(false); return; }
      await supabase.from("members").upsert({ session_id: code, name: nameInput.trim(), score: 0 }, { onConflict: "session_id,name", ignoreDuplicates: true });
      const { data: m } = await supabase.from("members").select("*").eq("session_id", code);
      const { data: i } = await supabase.from("items").select("*").eq("session_id", code).order("logged_at", { ascending: true });
      setSessionData(s); setMembersData(m || []); setItemsData(i || []);
      setCurrentSession(code); setCurrentUser(nameInput.trim()); setScreen("session"); setError("");
    } catch (e) { setError("Erreur : " + (e.message || JSON.stringify(e))); }
    setLoading(false);
  }
  ...
  async function addItem(item) {
    ...
    try {
      await supabase.from("items").insert({ session_id: currentSession, member_name: currentUser, item_id: item.id, emoji: item.emoji, item_name: item.name, points: item.points, logged_at: Date.now() });
      await supabase.from("members").update({ score: newScore }).eq("session_id", currentSession).eq("name", currentUser);
      await pollSession();
  ...
  async function removeLast() {
    ...
    try {
      await supabase.from("items").delete().eq("id", last.id);
      await supabase.from("members").update({ score: newScore }).eq("session_id", currentSession).eq("name", currentUser);
      await pollSession();
      showToast("Annulé ✓");
    } catch (e) { showToast("Erreur 😕"); }
  ...
  async function endSessionNow() {
    try {
      await supabase.from("sessions").update({ ended: true, ends_at: Date.now() }).eq("id", currentSession);
  ...
  async function saveRound() {
    ...
    try {
      await supabase.from("rounds").upsert({ id, ...roundForm, position });
      const { data: r } = await supabase.from("rounds").select("*").order("position", { ascending: true });
      setRounds(r);
  ...
  async function moveRound(id, dir) {
    ...
    try {
      await supabase.from("rounds").update({ position: idx }).eq("id", newR[idx].id);
      await supabase.from("rounds").update({ position: target }).eq("id", newR[target].id);
      ...
      <div style={S.root}>
        ...
        <div style={S.screen}>
          ...
          <div style={S.editorCard}>
            ...
            <textarea style={S.textarea} placeholder="Consigne ou texte" value={roundForm.prompt} onChange={e => setRoundForm({ ...roundForm, prompt: e.target.value })} rows={3} />
